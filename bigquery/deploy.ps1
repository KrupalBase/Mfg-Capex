...
if (-not $FLASK_SECRET) { $FLASK_SECRET = [guid]::NewGuid().ToString() }

$ENV_COMMON = "GCS_BUCKET=$BUCKET"
if ($CLIENT_ID -and $CLIENT_SECRET) {
    $ENV_COMMON += ",GOOGLE_CLIENT_ID=$CLIENT_ID,GOOGLE_CLIENT_SECRET=$CLIENT_SECRET,FLASK_SECRET_KEY=$FLASK_SECRET"
}
...
if ($Seed) {
    Write-Host "==> Uploading local data/ to gs://$BUCKET/ ..."
