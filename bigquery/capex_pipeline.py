...
def step3_load_stations() -> tuple[list[dict], list[dict]]:
    ...
    if not EXCEL_FILE.exists():
        print(f"  WARNING: Excel not found at {EXCEL_FILE}, skipping station mapping.")
        return [], []
...
def step9_export(df: pd.DataFrame, stations: list[dict]) -> None:
    """Export all CSVs to data/ directory."""
    _step(9, "Export all CSVs")
    ...
    col_order = [
        "source", "po_number", "date_order", "po_state", "vendor_name", "vendor_ref",
    ...
    if missing:
        missing_rows = pd.DataFrame([{
            ...
            "actual_with_tax": 0, "odoo_spend": 0, "ramp_spend": 0,
        } for sid in missing])
    ...
    col_order_station = [
        ...
        "odoo_spend", "ramp_spend", "actual_with_tax", "line_count",
    ]
    ...
    # --- spares_catalog.csv ---
    odoo_spend = spend[(spend["source"] == "odoo") & (spend["item_description"] != "")].copy()
    odoo_spend["_subtotal"] = pd.to_numeric(odoo_spend["price_subtotal"], errors="coerce").fillna(0)
    odoo_spend["_qty"] = pd.to_numeric(odoo_spend["product_qty"], errors="coerce").fillna(0)
    odoo_spend["_unit"] = pd.to_numeric(odoo_spend["price_unit"], errors="coerce").fillna(0)

    if not odoo_spend.empty:
        spares = odoo_spend.groupby("item_description").agg(
            ...
            po_numbers=("po_number", lambda x: ", ".join(sorted(set(x)))),
...
def step10_summary(df: pd.DataFrame) -> None:
...
def main() -> None:
    ...
    step9_export(unified, stations)
    step10_summary(unified)
